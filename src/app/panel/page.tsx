  ...
  return (
    <div className="panel-page" style={{ maxWidth: 1320, margin: "0 auto" }}>
      ...
      <section aria-labelledby="pacientes-title" style={{ ...estilos.tarjeta, overflow: "hidden", marginBottom: 25 }}>
        <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 15 }}><div><h2 id="pacientes-title" style={{ margin: 0, fontSize: 16 }}>Hospitalización actual</h2><p style={{ margin: "3px 0 0", color: "var(--muted)", fontSize: 11 }}>Pacientes que actualmente no tienen un egreso registrado.</p></div><span style={{ color: "var(--muted)", fontSize: 10, fontWeight: 700, whiteSpace: "nowrap" }}>{pacientes.length} activo{pacientes.length === 1 ? "" : "s"}</span></div>
        {cargando ? <div style={{ padding: 22, color: "var(--muted)", fontSize: 12 }}>Cargando pacientes hospitalizados...</div> : pacientes.length === 0 ? <div style={{ padding: 22, color: "var(--muted)", fontSize: 12 }}>No hay pacientes hospitalizados actualmente.</div> : <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}><thead><tr style={{ background: "#f8fafb" }}>{["HC", "Paciente", "Ubicación", "Médico", "Diagnóstico", "Recursos"].map((encabezado) => <th key={encabezado} style={{ padding: "9px 11px", textAlign: "left", color: "var(--muted)", fontSize: 9, textTransform: "uppercase", letterSpacing: ".06em", whiteSpace: "nowrap" }}>{encabezado}</th>)}</tr></thead><tbody>{pacientes.map((paciente) => <tr key={paciente.ingresoId} style={{ borderTop: "1px solid #edf1f3" }}><td style={{ padding: "10px 11px", fontWeight: 750, whiteSpace: "nowrap" }}>{paciente.hc}</td><td style={{ padding: "10px 11px", minWidth: 170 }}><strong>{paciente.paciente}</strong></td><td style={{ padding: "10px 11px", minWidth: 160 }}><strong>{paciente.servicio.nombre}</strong><div style={{ color: "var(--muted)", marginTop: 2 }}>{paciente.especialidad.nombre} · Cama {paciente.cama.numero}</div></td><td style={{ padding: "10px 11px", minWidth: 155 }}>{paciente.medico ? <><strong>{paciente.medico.nombre}</strong><small style={{ display: "block", marginTop: 2, color: "var(--muted)" }}>CMP {paciente.medico.cmp}</small></> : <span style={{ color: "var(--muted)" }}>No registrado</span>}</td><td style={{ padding: "10px 11px", minWidth: 220 }}>{paciente.diagnostico.codigo && <strong>{paciente.diagnostico.codigo}</strong>}{paciente.diagnostico.descripcion && <div style={{ marginTop: 2, color: "var(--muted)", lineHeight: 1.35 }}>{paciente.diagnostico.descripcion}</div>}{!paciente.diagnostico.codigo && !paciente.diagnostico.descripcion && "—"}</td><td style={{ padding: "10px 11px", whiteSpace: "nowrap" }}>{paciente.usaOxigeno && <span style={{ marginRight: 5, padding: "3px 6px", borderRadius: 999, background: "var(--primary-light)", color: "var(--primary-dark)", fontSize: 9, fontWeight: 800 }}>O₂</span>}{paciente.usaVentilador && <span style={{ padding: "3px 6px", borderRadius: 999, background: "var(--secondary-light)", color: "var(--secondary)", fontSize: 9, fontWeight: 800 }}>VENT</span>}{!paciente.usaOxigeno && !paciente.usaVentilador && <span style={{ color: "var(--muted)" }}>—</span>}</td></tr>)}</tbody></table></div>}
      </section>
