...
function fechaCorta(fecha: string) { const v = new Date(fecha); return Number.isFinite(v.getTime()) ? v.toLocaleDateString("es-PE") : "—"; }
...
export default function EgresosReportPage() {
  ...
  const consultar = async () => {
    ...
    try {
      ...
      if (servicio !== "todos") p.set("servicioId", servicio); if (tipo !== "todos") p.set("tipo", tipo);
  ...
  const limpiar = () => { setDesde(fechaLocal(inicioMes)); setHasta(fechaLocal(hoy)); setServicio("todos"); setTipo("todos"); setBusqueda(""); setTimeout(consultar, 0); };
